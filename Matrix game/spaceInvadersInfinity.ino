...
void showEnemies(Enemie *enemies) {
  for(int i = 0; i < numberOfEnemies; i++) {
    ...
    if(millis() - enemies[i].createdTime > enemies[i].spawnTime && enemies[i].posX != -1)
      if(enemies[i].posY < 8) {
        lc.setLed(1, enemies[i].posY - 1, enemies[i].posX - 1, true);
        lc.setLed(1, enemies[i].posY, enemies[i].posX, true);
        lc.setLed(1, enemies[i].posY - 1, enemies[i].posX + 1, true);
      }
      else {
        lc.setLed(0, enemies[i].posY % 8 - 1, enemies[i].posX - 1, true);
        lc.setLed(0, enemies[i].posY % 8, enemies[i].posX, true);
        lc.setLed(0, enemies[i].posY % 8 - 1, enemies[i].posX + 1, true);
      }
  }
}
  ...
  for(int i = 0; i < numberOfEnemies; i++) {
    if(millis() - enemies[i].createdTime > enemies[i].spawnTime) {
      if(millis() - enemies[i].movementTime > enemies[i].movementSpeed) {
        if(enemies[i].posY < 8) {
          lc.setLed(1, enemies[i].posY - 1, enemies[i].posX - 1, false);
          lc.setLed(1, enemies[i].posY, enemies[i].posX, false);
          lc.setLed(1, enemies[i].posY - 1, enemies[i].posX + 1, false);
        }
        else {
          lc.setLed(0, enemies[i].posY % 8 - 1, enemies[i].posX - 1, false);
        lc.setLed(0, enemies[i].posY % 8, enemies[i].posX, false);
        lc.setLed(0, enemies[i].posY % 8 - 1, enemies[i].posX + 1, false);
        }
...
void updateBullets() {
  for(int i = 0; i < noOfBullets; i++) {
    if(bullets[i].posX != -1 && millis() - bullets[i].movementDelay > bulletDelay) {
      if(bullets[i].posY > 7)
        lc.setLed(0, bullets[i].posY % 8, bullets[i].posX, false);
      else 
        lc.setLed(1, bullets[i].posY, bullets[i].posX, false);
...
void showBullets() {
  for(int i = 0; i < noOfBullets; i++) {
    ...
    if(bullets[i].posX != -1)
      if(bullets[i].posY > 7)
        lc.setLed(0, bullets[i].posY % 8, bullets[i].posX, true);
      else lc.setLed(1, bullets[i].posY, bullets[i].posX, true);
  }
}
...
void showPlayer() {
  ...
  lc.setLed(0, 7, playerPos + 1, true); 
  lc.setLed(0, 6, playerPos - 1, true);
  lc.setLed(0, 6, playerPos + 1, true);
  
  lc.setLed(0, 7, playerPos, false);
  lc.setLed(0, 7, playerPos - 1, false);
  lc.setLed(0, 7, playerPos + 1, false); 
  lc.setLed(0, 6, playerPos - 1, false);
  lc.setLed(0, 6, playerPos + 1, false);
}
...
void getPlayerMovement() {
  ...
  if(xValue > maxThreshold && millis() - movementTime >= movementDelay) {
    playerPos++;
    movementTime = millis();
  }  
  else if(xValue < minThreshold && millis() - movementTime >= movementDelay) {
    playerPos--;
    movementTime = millis();
  }
